...
from scipy.signal import filtfilt, firwin, resample, iirnotch
...
def read_edf(edf_path, csv_path, ns2_path=None):
        ...
        if s in stages_map.keys():
            ...
            if stopping:
                if starting != stopping:
                    ...
                    if length < 0:
                        print(f'Overlapping labels, length {length}, trusting start time over duration')
                        stages = stages[:length]
                    else:
                        print(f'Missing labels, length {length}')
    ...
    b, a = iirnotch(w0=50/(old_fs/2), Q=30)
    emg = filtfilt(b, a, emg)
    b, a = iirnotch(w0=100/(old_fs/2), Q=30)
    emg = filtfilt(b, a, emg)
        ...
        for subj in subjects:
            for night in nights:
                ...
                if not os.path.exists(records_dir):
                    logger.warning(f"Record directory {records_dir} does not exist. Skipping subject {subj} night {night}.")
                    continue

                if subj == 'Subj004' and night == 'Night001':
                    continue
