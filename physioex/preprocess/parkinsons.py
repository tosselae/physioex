...
POSSIBLE_EEG_CHANNELS = ["EEG C4-A1", "EEG C4-REF", "EEG C3-A2", "EEG C3-REF"]

POSSIBLE_EOG_CHANNELS = [("EOG LEFT", "EOG RIGHT"), ("EOG2", "EOG1")]
...
def read_edf(edf_path, tsv_path):
    ...
    stages = df[2].tolist()
    stages = [stages_map[s] if s in stages_map else -1 for s in stages]
...
class ParkinsonsPreprocessor(Preprocessor):
    ...
    def get_subjects_records(self) -> List[str]:
        table_path = os.path.join(
            self.root_folder, "Parkinson_data", "Target_sleep_demographic.csv"
        )
        ...
        for idx, row in table.iterrows():
            ...
            records_dir = os.path.join(
                ...
                "Parkinson_data",
                "Data",
                str(subject_id) + "_nap" if not self.night else str(subject_id),
            )
            # check if the directory exists if not continue
            if not os.path.exists(records_dir):
                logger.warning(
                    f"Record directory {records_dir} does not exist. Skipping subject {subject_id}."
                )
                continue
    ...
    def read_subject_record(self, record: str) -> Tuple[np.array, np.array]:
        ...
        signal, labels = read_edf(
            os.path.join(record, edf_path), os.path.join(record, tsv_path)
        )
...
def global_split(data_folder):
    for group in ["HOA", "PD"]:
        table_path_nap = os.path.join(
            data_folder, "parkinsons4", "nap", group, "table.csv"
        )
        table_nap = pd.read_csv(table_path_nap)
        table_path_night = os.path.join(
            data_folder, "parkinsons4", "night", group, "table.csv"
        )
        ...
        table_nap.loc[
            table_nap["record_id"].isin(test_subjects[fold]), f"fold_{fold}"
        ] = "test"
        ...
        table_night.loc[
            table_night["record_id"].isin(test_subjects[fold]), f"fold_{fold}"
        ] = "test"
...
if __name__ == "__main__":
    ParkinsonsPreprocessor(
        data_folder="/home/coder/sleep/sleep-data/", night=True, healthy=True
    ).run()
    ParkinsonsPreprocessor(
        data_folder="/home/coder/sleep/sleep-data/", night=True, healthy=False
    ).run()
    ParkinsonsPreprocessor(
        data_folder="/home/coder/sleep/sleep-data/", night=False, healthy=True
    ).run()
    ParkinsonsPreprocessor(
        data_folder="/home/coder/sleep/sleep-data/", night=False, healthy=False
    ).run()
