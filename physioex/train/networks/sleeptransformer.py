...
class MiceTransformer(SleepModule):
    def __init__(self, module_config=module_config):
        ...
        self.wacc = tm.Accuracy(task="multiclass", num_classes=3, average="weighted")
        self.macc = tm.Accuracy(task="multiclass", num_classes=3, average="macro")
        self.wf1 = tm.F1Score(task="multiclass", num_classes=3, average="weighted")
        self.mf1 = tm.F1Score(task="multiclass", num_classes=3, average="macro")
        self.ck = tm.CohenKappa(task="multiclass", num_classes=3)
        self.pr = tm.Precision(task="multiclass", num_classes=3, average="weighted")
        self.rc = tm.Recall(task="multiclass", num_classes=3, average="weighted")
        self.cm = {
            "cm": tm.ConfusionMatrix(
                task="multiclass", num_classes=3, normalize=None
            ).cuda()
        }
        self.cm["cm5"] = tm.ConfusionMatrix(
            task="multiclass", num_classes=5, normalize=None
        ).cuda()
        ...
        self.mice_to_human_encoder = nn.TransformerEncoder(t_layer, num_layers=4)
    ...
    def compute_loss(
        ...
        if log_metrics:
            self.cm["cm5"].update(
                outputs.reshape(-1, outputs.shape[-1]), targets.flatten()
            )
        ...
        return super(MiceTransformer, self).compute_loss(
            embeddings, outputs, targets, log, log_metrics
        )
