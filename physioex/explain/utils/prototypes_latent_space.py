...
def get_groups(
    datamodule: PhysioExDataModule, dataset_idx: torch.Tensor, subject_id: torch.Tensor
) -> List[str]:
    groups = []
...
def subject_density(
    prototype_predictions: np.ndarray[np.int_],
    channels: List[str],
    n_proto: List[int],
    subjects: np.ndarray[np.int_],
    results_path: str,
    attribute: str = "subjects",
) -> None:
    ...
    sns.barplot(
        x=df[attribute].value_counts().index,
        y=df[attribute].value_counts(normalize=True).values,
        palette="viridis",
    )
    ...
    for c_idx, c in enumerate(channels):
        ratio_table = (
            df.groupby(c)[attribute].value_counts(normalize=True).unstack().fillna(0)
        )
        fig, axes = plt.subplots(
            int(np.ceil(np.sqrt(n_proto[c_idx]))),
            int(np.floor(np.sqrt(n_proto[c_idx]))),
            figsize=(25, 25),
            dpi=300,
        )
        for i in range(axes.size):
            ...
            if i not in ratio_table.index:
                ...
                ax.pie(
                    ratio_table.loc[i],
                    labels=ratio_table.loc[i].index,
                    autopct="%1.1f%%",
                    startangle=90,
...
def count_transitions(
    prototype_predictions: np.ndarray[np.int_], transition_matrix: np.ndarray[np.int_]
):
    sequence_center = prototype_predictions.shape[1] // 2 + 1
    prototype_predictions = prototype_predictions[
        :, sequence_center - 1 : sequence_center + 1, 0
    ]
...
def average_psd(channels: List[str], mean_psd: dict, results_path: str) -> None:
    ...
    for c_idx, c in enumerate(channels):
        ...
        fig, axes = plt.subplots(
            int(np.ceil(np.sqrt(n_proto_c))),
            int(np.floor(np.sqrt(n_proto_c))),
            figsize=(5 * int(np.sqrt(n_proto_c)), 4 * int(np.sqrt(n_proto_c))),
            dpi=300,
            tight_layout=True,
        )
        fig_linear, axes_linear = plt.subplots(
            int(np.ceil(np.sqrt(n_proto_c))),
            int(np.floor(np.sqrt(n_proto_c))),
            figsize=(5 * int(np.sqrt(n_proto_c)), 4 * int(np.sqrt(n_proto_c))),
            dpi=300,
...
def prototype_density(
    ...
    for c_idx, c in enumerate(channels):
        ratio_table = (
            df.groupby(c)["y"].value_counts(normalize=True).unstack().fillna(0)
        )
        fig, axes = plt.subplots(
            int(np.ceil(np.sqrt(n_proto[c_idx]))),
            int(np.floor(np.sqrt(n_proto[c_idx]))),
        ...
        for i in range(axes.size):
            ...
            if i not in ratio_table.index:
                ...
                ax.pie(
                    ratio_table.loc[i].sort_index(),
                    labels=ratio_table.loc[i].sort_index().index,
                    autopct="%1.1f%%",
                    startangle=90,
                    textprops={"fontsize": 25},
                ...
                ax.set_title(
                    f"Prototype {i}, {((df[c] == i).sum() / len(y) * 100):.2f}%",
                    fontsize=25,
                )
        plt.tight_layout()
    ...
    for c_idx, c in enumerate(channels):
        if isinstance(axes, np.ndarray):
            ...
            ax = axes
        ratio_table = (
            df.groupby("y")[c].value_counts(normalize=True).unstack().fillna(0).T
        )
        ratio_table = ratio_table.reset_index()
        melted_df = ratio_table.melt(
            id_vars=c, var_name="Stage", value_name="Sensitivity"
        )
            ...
            if proto not in melted_df[c].unique():
                melted_df = pd.concat(
                    [
                        melted_df,
                        pd.DataFrame(
                            {c: [proto], "Stage": [np.nan], "Sensitivity": [np.nan]}
                        ),
                    ],
                    ignore_index=True,
                )
        sns.barplot(
            ...
            hue_order=sorted(melted_df["Stage"].dropna().unique()),
        )
        ax.set_title(c)
    ...
    for c_idx, c in enumerate(channels):
        for idx in range(n_proto[c_idx]):
            bar_data.append(
                {"Channel": c, "Prototype": idx, "Count": value_counts[c].get(idx, 0)}
            )
...
def visualize(
    ...
    learned_prototypes = [
        chan_codebook.codebook.cpu().detach().numpy()
        for chan_codebook in model.nn.prototype
    ]
    n_proto = [chan_codebook.shape[0] for chan_codebook in learned_prototypes]
    channels = datamodule_kwargs["selected_channels"]
    n_channels = len(channels)
    assert len(n_proto) == n_channels, (
        "Number of prototypes must match number of channels"
    )
    ...
    for c_idx, c in enumerate(channels):
        ...
        transition_matrices[c] = np.zeros(
            (n_proto[c_idx], n_proto[c_idx]), dtype=np.int_
        )
    ...
    for _, test_datamodule in enumerate(datamodule):
        ...
        for batch in tqdm(range(n_batches), desc="Getting prototypes"):
            ...
            for c_idx, c in enumerate(channels):
                transition_matrices[c] = count_transitions(
                    proto_idx_[:, :, c_idx], transition_matrices[c]
                )

            x_ = (
                x_ * test_datamodule.dataset.readers[0].reader.std
                + test_datamodule.dataset.readers[0].reader.mean
            )

            embeddings_ = (
                embeddings_[:, sequence_center, :, :, :].detach().cpu().numpy()
            )  # take center element of the sequence
        ...
        for c_idx, c in enumerate(channels):
            transition_matrices[c] = transition_matrices[c] / np.sum(
                transition_matrices[c], axis=1, keepdims=True
            )
            transition_matrices[c] = np.nan_to_num(transition_matrices[c], nan=0)
        fig, axes = plt.subplots(
            1,
            n_channels,
            figsize=(n_channels * max(n_proto) * 0.6, max(n_proto) * 0.6),
            dpi=300,
        )
        ...
        for c_idx, c in enumerate(channels):
            if isinstance(axes, np.ndarray):
                ...
                ax = axes
            sns.heatmap(
                transition_matrices[c], annot=True, fmt=".2f", cmap="Blues", ax=ax
            )
        ...
        for c_idx, c in enumerate(channels):
            embeddings[c] = np.concatenate(
                (np.squeeze(embeddings[c]), learned_prototypes[c_idx]), axis=0
            )
        ...
        for k, v in projectors.items():
            ...
            for c_idx, c in enumerate(channels):
                ...
                sns.scatterplot(
                    ax=axes[0, c_idx],
                    x=embeddings_2d[: -n_proto[c_idx], 0],
                    y=embeddings_2d[: -n_proto[c_idx], 1],
                    hue=y,
                ...
                for p in range(n_proto[c_idx]):
                    axes[0, c_idx].text(
                        embeddings_2d[-n_proto[c_idx] + p, 0],
                        embeddings_2d[-n_proto[c_idx] + p, 1],
                        str(p),
                        fontsize=12,
                        weight="bold",
                        color="black",
                ...
                xlim = axes[
                    0, c_idx
                ].get_xlim()  # Save current limits because voronoi_plot_2d changes them
                ...
                axes[0, c_idx].set_xlim(
                    (xlim[0], xlim[1] * 1.25)
                )  # Restore the original limits and make space for legend
                ...
                sns.scatterplot(
                    ...
                    y=embeddings_2d[: -n_proto[c_idx], 1],
                    hue=proto_idx[:, c_idx],
                    palette=sns.color_palette("bright", n_proto[c_idx]),
                )
                for p in range(n_proto[c_idx]):
                    axes[1, c_idx].text(
                        embeddings_2d[-n_proto[c_idx] + p, 0],
